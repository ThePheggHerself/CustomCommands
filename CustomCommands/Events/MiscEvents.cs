...
namespace CustomCommands.Events
{
    public class MiscEvents
    {
        ...
        {
            if (!obj.TryGetComponent<FakeRagdoll>(out FakeRagdoll comp) && obj.Info.Handler is UniversalDamageHandler uDH && uDH.TranslationId == DeathTranslations.PocketDecay.Id)
            {
                try
                {
